...
| Проверяемый канал | `@test_gift_channel_bot` |
| Ссылка на канал | `https://t.me/test_gift_channel_bot` |
...
flowchart LR
    ...
    F --> G["getChatMember: проверка @test_gift_channel_bot"]
    G -->|"не подписан"| E
...
- **«Подписаться на канал»** — обычная ссылка на `https://t.me/test_gift_channel_bot`;
- **«Проверить подписку»** — явный запуск проверки пользователем.

После нажатия бот вызывает единственный запрос `getChatMember(@test_gift_channel_bot, Telegram ID пользователя)`.
...
TELEGRAM_CHANNEL_ID=@test_gift_channel_bot
TELEGRAM_CHANNEL_URL=https://t.me/test_gift_channel_bot
...
Бот добавлен администратором канала `@test_gift_channel_bot`, поэтому Telegram гарантированно разрешает проверять других участников через `getChatMember`.
...
| Подписка не находится | проверить фактическую подписку, `@test_gift_channel_bot` в `.env` и наличие бота среди администраторов канала |
...
- [ ] Убедиться, что бот остаётся администратором `@test_gift_channel_bot` и `getChatMember` работает.
